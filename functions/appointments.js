...
// Public endpoint — no auth required for patient bookings
...
export async function onRequestGet({ env }) {
  try {
    ...
    console.error('GET /appointments error:', e.message);
...
export async function onRequestDelete({ request, env }) {
  ...
  try {
    const r = await env.DB.prepare(`UPDATE appointments SET status='cancelled' WHERE id=?`).bind(id).run();
    if (!r.meta?.changes) return text('Not found', 404);
    return json({ success: true, id });
  } catch (e) {
    console.error('DELETE /appointments error:', e.message);
    return text('Internal error', 500);
  }
}
