...
export async function onRequestGet(ctx) {
  ...
  const { results: prescriptions } = await ctx.env.DB.prepare(
    `SELECT rx.*, u.name AS doctor_name
     FROM prescriptions rx LEFT JOIN users u ON u.id = rx.doctor_id
     WHERE rx.patient_id = ? ORDER BY rx.rx_date DESC LIMIT 20`
  ).bind(id).all();

  const { results: procedures } = await ctx.env.DB.prepare(
    `SELECT pr.*, u.name AS doctor_name
     FROM procedures pr LEFT JOIN users u ON u.id = pr.doctor_id
...
export async function onRequestPut(ctx) {
  ...
  await ctx.env.DB.prepare(
    ...
    body.full_name, body.date_of_birth || null, body.gender || null, body.civil_status || null,
    body.contact_number || null, body.email || null, body.address || null, body.city || null,
    body.emergency_contact_name || null, body.emergency_contact_number || null,
    body.emergency_relation || null, body.blood_type || null,
    body.known_allergies || null, body.medical_history_notes || null,
    body.philhealth_no || null, body.branch || 'jaro', id
  ).run();
